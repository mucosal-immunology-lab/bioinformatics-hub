pmp_preprocess <- function(pos_df, neg_df, metadata = NULL, samples_key = 'Sample', intens_cols = NULL, info_cols = NULL,
  ...
  metab_pos_df <- metab_pos[, intens_cols] %>% mutate(across(where(is.character), as.numeric))
  metab_neg_df <- metab_neg[, intens_cols] %>% mutate(across(where(is.character), as.numeric))
  metab_pos_counts <- as.matrix(metab_pos_df)
  metab_neg_counts <- as.matrix(metab_neg_df)
